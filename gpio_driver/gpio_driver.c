...
#define TIMER_SEC1 5
#define TIMER_NANO_SEC1 0

MODULE_LICENSE("Dual BSD/GPL");         ///< The license type -- this affects available functionality
MODULE_AUTHOR("Sasa Pejasinovic"); 
MODULE_AUTHOR("Dimitrije Obradovic");   ///< The author -- visible when you use modinfo
MODULE_DESCRIPTION("A simple Linux char driver for the Braille ASCII");  ///< The description -- see modinfo
...
void setPins(char);
static long A=2250;
module_param(A,long,0000);
static long B=1750;
module_param(B,long,0000);

static struct hrtimer blink_timer;
static struct hrtimer change_timer;
static ktime_t restart;
static ktime_t kt;
static ktime_t kt1;
static struct RingBuffer buffer;
static long seconds1;
static long ms1;
static long seconds;
static long ms;
static char letter;
...
static enum hrtimer_restart change_timer_callback(struct hrtimer *param)
{
 ...
 */
static ssize_t gpio_driver_read(struct file *filp, char *b, size_t len, loff_t *f_pos)
{
    	char freeSpace;
 ...
 */
static ssize_t gpio_driver_write(struct file *filp, const char *usBuffer, size_t len, loff_t *f_pos)
{
   	char lett;
	if(get_user(lett,usBuffer))
		return -EFAULT;

	ringBufPutChar(&buffer,lett);
	buffer.counter++;
	return len;
}
void setPins(char letter)
{
	ClearGpioPin(GPIO_06);
	ClearGpioPin(GPIO_07);
	ClearGpioPin(GPIO_08);
